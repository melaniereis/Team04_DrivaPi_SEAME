...
save_coverage_for_aggregation() {
    ...
    # PROJECT_ROOT is set to dc-motor dir, so we need to go up 3 levels to reach drivapi/
    ABSOLUTE_PROJECT_ROOT="$(cd "${PROJECT_ROOT}/../.." && pwd)"
    PERSISTENT_COVERAGE_DIR="${ABSOLUTE_PROJECT_ROOT}/build/coverage/dc-motor"
