...
HAL_StatusTypeDef PCA9685_SetPWM(I2C_HandleTypeDef *hi2c, uint16_t addr, uint8_t channel, uint16_t on, uint16_t off) {
	...
	{
		if (i2c_debug) 
			UART_Printf("PCA9685_SetPWM: wrote PWM to 0x%02X reg=0x%02X\r\n", addr, reg);
		return HAL_OK;
	}

	uint32_t hal_err_mem = HAL_I2C_GetError(hi2c);
	if (i2c_debug)
		UART_Printf("PCA9685_SetPWM: Mem_Write failed addr=0x%02X HALerr=0x%08lX\r\n", addr, (unsigned long)hal_err_mem);
	...
	if (status != HAL_OK)
	{
		...
		if (i2c_debug)
			UART_Printf("PCA9685_SetPWM: write to 0x%02X failed status=%d HALerr=0x%08lX\r\n", addr, (int)status, (unsigned long)hal_err);
	}
	return status;
}
